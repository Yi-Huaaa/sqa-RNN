...
class RNNwavefunction(object):
    ...
    def log_probability(self,samples,inputdim):
        ...
        with self.graph.as_default():
            ...
            log_probs2 = tf.reduce_sum(input_tensor=tf.math.log(tf.reduce_sum(input_tensor=tf.multiply(probs,one_hot_samples),axis=2)),axis=1)

            return tf.math.log(0.5*(tf.exp(log_probs1)+tf.exp(log_probs2))) #We take the average of P1 and P2 to impose the parity symmetry
