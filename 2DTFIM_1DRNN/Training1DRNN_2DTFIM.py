...
def run_2DTFIM(numsteps = 2*10**4, systemsize_x = 5, systemsize_y = 5, Bx = +2, num_units = 50, num_layers = 1, numsamples = 500, learningrate = 1e-3, seed = 333):
    ...
    #tf2.compat.v1.set_random_seed(seed)  # tensorflow pseudo-random generator
...
# Intitializing the RNN-----------
    ...
    with wf.graph.as_default():
        ...
        samples_placeholder=tf.compat.v1.placeholder(dtype=tf.int32,shape=[numsamples_,Nx*Ny]) #the samples_placeholder are the samples of all of the spins
    ...
    with tf.compat.v1.variable_scope(wf.scope,reuse=tf.compat.v1.AUTO_REUSE):
        with wf.graph.as_default():
            ...
            samp=tf.compat.v1.placeholder(dtype=tf.int32,shape=[numsamples,Nx*Ny])
            ...
            cost = tf.reduce_mean(input_tensor=tf.multiply(log_probs_,tf.stop_gradient(Eloc))) - tf.reduce_mean(input_tensor=tf.stop_gradient(Eloc))*tf.reduce_mean(input_tensor=log_probs_)
